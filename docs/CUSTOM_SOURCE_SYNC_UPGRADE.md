...
本文记录 `Tingfe/lx-music-sync-server` 的 `v2.1.8` 定制改动，以及 Docker 部署的升级方式。
...
```text
tingfe/lx-music-sync-server:v2.1.8
```
...
```yaml
image: tingfe/lx-music-sync-server:v2.1.8
```
...
```bash
docker pull tingfe/lx-music-sync-server:v2.1.8
